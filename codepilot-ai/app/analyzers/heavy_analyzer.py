...
class HeavyPythonAnalyzer(ast.NodeVisitor):
    """Fast, deterministic, AI-free static analyzer for large Python files.

    It parses once and performs bounded linear AST/token scans. It never imports
    or executes the analyzed source.
    """
    ...
    def __init__(self, *, long_function: int = 60, long_class: int = 250,
                 complexity: int = 10, nesting: int = 5, parameters: int = 8):
        ...
        self.imported_names: dict[str, tuple[int, int]] = {}
        self.function_stack: list[FunctionStats] = []
        self.nesting = 0
        self.max_nesting = 0
        self._source_lines: list[str] = []

    def add(self, node: ast.AST, severity: str, category: str, code: str, message: str, suggestion: str):
    ...
    def analyze(self, source: str, filename: str = "<string>") -> dict:
        ...
        try:
            ...
            return self._report(metrics, filename, syntax_error={"line": exc.lineno, "column": exc.offset, "message": exc.msg})

        self.visit(tree)
        self._post_import_checks()
        self._line_checks()
        self.findings.sort(key=lambda x: (x.line, x.column, -self._severity_weight(x.severity)))
        metrics.update({
        ...
        return self._report(metrics, filename, syntax_error=None)
    ...
    def _metrics(self, source: str) -> dict:
        ...
        try:
            ...
            token_count = sum(t.type not in (tokenize.ENCODING, tokenize.ENDMARKER) for t in tokens)
            comments = sum(t.type == tokenize.COMMENT for t in tokens)
    ...
    def _report(self, metrics: dict, filename: str, syntax_error: dict | None) -> dict:
        if syntax_error is None:
            # ast walk already happened; count findings and approximate node count from visitors.
            metrics["ast_nodes"] = getattr(self, "_ast_nodes", 0)
    ...
    def generic_visit(self, node: ast.AST):
        self._ast_nodes = getattr(self, "_ast_nodes", 0) + 1
    ...
    def _visit_function(self, node: ast.FunctionDef | ast.AsyncFunctionDef):
        ...
        if positional > self.parameter_limit:
            self.add(node, "medium", "maintainability", "PY-MANY-PARAMS",
                     f"تابع '{node.name}' پارامترهای زیادی ({positional}) دارد.",
                     "پارامترها را با یک object/config یا چند تابع کوچک‌تر سازمان‌دهی کنید.")
        ...
        if stats.complexity > self.complexity_limit:
            self.add(node, "high", "complexity", "PY-HIGH-COMPLEXITY",
                     ...
                     "شرط‌ها و حلقه‌های تو‌در‌تو را کاهش و منطق را به توابع کوچک‌تر تقسیم کنید.")
        if stats.max_nesting > self.nesting_limit:
            self.add(node, "high", "complexity", "PY-DEEP-NESTING",
                     ...
                     "از guard clause، استخراج تابع و ساختارهای ساده‌تر استفاده کنید.")
        self.function_stack.pop()
        self.nesting = old_nesting

    def _branch(self, node: ast.AST, amount: int = 1):
        if self.function_stack:
            self.function_stack[-1].complexity += amount
            self.function_stack[-1].branches += amount

    def _nested(self, node: ast.AST, body: list[ast.stmt], extra: list[ast.stmt] | None = None):
        self._branch(node)
        self.nesting += 1
    ...
    def visit_If(self, node: ast.If):
        self._nested(node, node.body, node.orelse)
        self.generic_visit(node.test)

    def visit_For(self, node: ast.For):
        self._nested(node, node.body, node.orelse)
        self.generic_visit(node.target); self.generic_visit(node.iter)
    ...
    def visit_Try(self, node: ast.Try):
        self._branch(node, len(node.handlers) + (1 if node.orelse else 0))
        self.nesting += 1; self.max_nesting = max(self.max_nesting, self.nesting)
        for item in node.body + node.orelse + node.finalbody:
            self.visit(item)
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler):
        if node.type is None:
            self.add(node, "medium", "correctness", "PY-BARE-EXCEPT", "bare except تشخیص داده شد.", "exception مشخص و محدودتری را catch کنید.")
        self.generic_visit(node)

    def visit_With(self, node: ast.With):
        self._nested(node, node.body)
    ...
    def visit_Return(self, node: ast.Return):
        if self.function_stack:
            self.function_stack[-1].returns += 1
        self.generic_visit(node)

    def visit_Call(self, node: ast.Call):
        name = self._call_name(node)
        if name in {"eval", "exec"}:
            self.add(node, "critical", "security", "PY-EVAL", f"استفاده از {name}() می‌تواند اجرای کد پویا ایجاد کند.", "در صورت امکان آن را حذف و از parser/ساختار داده امن استفاده کنید.")
        if name == "os.system" or (name.startswith("subprocess.") and self._kw_bool(node, "shell", True)):
            self.add(node, "high", "security", "PY-SHELL", "اجرای command با shell فعال تشخیص داده شد.", "ورودی را مستقیم به argv بدهید و shell=True را حذف کنید.")
        if name in {"pickle.load", "pickle.loads", "marshal.loads"}:
            self.add(node, "high", "security", "PY-UNSAFE-DESERIALIZE", f"deserialize ناامن با {name} تشخیص داده شد.", "فقط داده‌های قابل‌اعتماد را deserialize کنید یا از قالب امن‌تر استفاده کنید.")
        if name in {"hashlib.md5", "hashlib.sha1"}:
            self.add(node, "medium", "security", "PY-MD5", f"استفاده از {name} برای hash تشخیص داده شد.", "برای کاربردهای امنیتی از SHA-256 یا الگوریتم مدرن‌تر استفاده کنید.")
        ...
        if name.startswith("requests.") and self._kw_bool(node, "verify", False):
            self.add(node, "medium", "security", "PY-VERIFY-FALSE", "TLS certificate verification غیرفعال شده است.", "verify را فعال نگه دارید مگر در محیط کنترل‌شده و موقت.")
        self.generic_visit(node)
    ...
    @staticmethod
    def _kw_bool(node: ast.Call, key: str, value: bool) -> bool:
    ...
    def _has_kw(node: ast.Call, key: str) -> bool:
        return any(k.arg == key for k in node.keywords)
    ...
    def visit_Assign(self, node: ast.Assign):
        for target in node.targets:
            ...
            if any(self.SECRET_RE.search(name) for name in names) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str) and len(node.value.value) >= 8:
                self.add(node, "high", "security", "PY-HARDCODED-SECRET", "مقدار متنی شبیه secret/token به‌صورت hard-coded قرار گرفته است.", "secret را در environment/secret manager نگه دارید.")
        self.generic_visit(node)

    def _target_names(self, target: ast.AST) -> list[str]:
        if isinstance(target, ast.Name): return [target.id]
        if isinstance(target, (ast.Tuple, ast.List)): return [n for elt in target.elts for n in self._target_names(elt)]
        return []

    def visit_Assert(self, node: ast.Assert):
        self.add(node, "low", "correctness", "PY-ASSERT", "assert در کد اجرایی تشخیص داده شد.", "برای validation مهم از exception صریح استفاده کنید؛ assert با -O حذف می‌شود.")
        self.generic_visit(node)

    def visit_ClassDef(self, node: ast.ClassDef):
        self.classes += 1
        statements = len(node.body)
        if statements > self.long_class:
            self.add(node, "medium", "maintainability", "PY-LONG-CLASS", f"کلاس '{node.name}' بسیار بزرگ است ({statements} statement).", "مسئولیت کلاس را به چند component مشخص تقسیم کنید.")
        self.generic_visit(node)
    ...
    def _post_import_checks(self):
        for name, (line, col) in self.imported_names.items():
            if name not in self.used_names:
                ...
                self.add(node, "low", "imports", "PY-POSSIBLE-UNUSED-IMPORT", f"import '{name}' ظاهراً استفاده نشده است.", "اگر واقعاً لازم نیست آن را حذف کنید؛ dynamic access و __all__ را در نظر بگیرید.")
    ...
    def _line_checks(self):
        for index, line in enumerate(self._source_lines, 1):
            if len(line) > 120:
                ...
                severity = "medium" if len(line) > 200 else "low"
                self.add(node, severity, "style", "PY-LONG-LINE", f"خط {index} دارای {len(line)} کاراکتر است.", "خط را کوتاه یا expression را به چند بخش تقسیم کنید.")
            if line.rstrip() != line:
                node = ast.Constant(value=None, lineno=index, col_offset=max(0, len(line) - 1))
                self.add(node, "info", "style", "PY-TRAILING-WHITESPACE", "فضای خالی انتهای خط وجود دارد.", "فضای خالی انتهای خط را حذف کنید.")
            if "TODO" in line or "FIXME" in line:
                node = ast.Constant(value=None, lineno=index, col_offset=0)
                self.add(node, "info", "maintenance", "PY-TODO", "TODO/FIXME در فایل وجود دارد.", "در صورت اهمیت، آن را به issue/task قابل پیگیری تبدیل کنید.")
